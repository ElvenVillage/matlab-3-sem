...
g_a = @(x) nthroot(((5-12*x^2-3*x^4)/4), 3);
g_b = @(x) exp(-(x+1)^3);
...
plot_a_y = zeros(9);
for i = 1:9
    [ax, ak] = bisection(0.1, 0.7, f_a, eps(i));
    plot_a_y(i) = ak;
end
ya1 = ax;
plot(X, plot_a_y, '-o')
title('Зависимость кол-ва итераций от порядка точности \epsilon для алгебраического уравнения')
xlabel('\epsilon, {10^{-x}}')
ylabel('N')
...
yb1 = bx;
plot(X, plot_b_y, '-o')
title('Зависимость кол-ва итераций от порядка точности \epsilon для трансцендентного уравнения')
xlabel('\epsilon, {10^{-x}}')
ylabel('N')
